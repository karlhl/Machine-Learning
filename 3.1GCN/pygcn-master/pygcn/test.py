...
def PersonalRank(G, alpha, root, max_step):
    ...
    for k in range(max_step):
        tmp = {x: 0 for x in G.keys()}  # 取节点i和它的出边尾节点集合ri
        for i, ri in G.items():  # i是顶点。ri是与其相连的顶点极其边的权重
            ...
            for j, wij in ri.items():  # j是i的连接顶点，wij是权重
                ...
                tmp[j] += alpha * rank[i] / (1.0 * len(ri))  # 我们每次游走都是从root节点出发，因此root节点的权重需要加上(1 - alpha)
            # 在《推荐系统实践》上，作者把这一句放在for j, wij in ri.items()这个循环下，我认为是有问题。
            tmp[root] += (1 - alpha)
            rank = tmp
            end = time.time()
            print('use_time', end - begin)  # 输出方法一
            lst = sorted(rank.items(), key=lambda x: x[1], reverse=True)
            for ele in lst:                 print("%s:%.3f, \t" % (ele[0], ele[1]))  # 输出方法二
            # 输出每次迭代后各个节点的权重
            # print ('iter:  ' + str(k) + "\t",)
